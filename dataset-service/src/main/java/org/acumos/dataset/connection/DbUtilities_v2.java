...
import org.acumos.dataset.config.WebConfig;
...
public class DbUtilities_v2 {
	...
	public boolean updateDataSet(String user, DatasetModelGet dataset) throws DataSetException, IOException {
		...
		if(!updateDataset) { //failed to update
			...
			DataSetRestError err;
			if(isDatasetExists(dataset.getDatasetKey())) {
				String[] variables = {OWNED_BY};
				err = DataSetErrorList.buildError(ErrorListEnum.E_0003, variables, null, CmlpApplicationEnum.DATASET);
	...
	public boolean deleteDataSet(String user, String datasetKey) throws IOException, DataSetException {
		...
		if(!deleteDataset) {
			...
			if(isDatasetExists(datasetKey)) {
				String[] variables = {OWNED_BY};
				err = DataSetErrorList.buildError(ErrorListEnum.E_1003, variables, null, CmlpApplicationEnum.DATASET);
				throw new DataSetException(
						PLEASE_CHECK_DATASET_KEY_PROVIDED_AND_USER_PERMISSION_FOR_THIS_OPERATION, Status.UNAUTHORIZED.getStatusCode(), err);
