...
import org.acumos.service.schema.DatasetModel;
...
public class DatasetController {
	...
	@Autowired
	RestDatasetSvcImplV2 service;
	
	@ApiOperation(value = RESPOND_A_LIST_OF_DATASET_DETAILS_BASED_ON_DATASET_KEY_OR_TEXT_SEARCH, notes = RETURNS_ONE_OR_MORE_JSON_DATASETS_AS_PER_THE_CRITERIA_PROVIDED, response = String.class)
	...
	@ApiOperation(value = RESPOND_DATASET_DETAILS_BASED_ON_THE_DATASET_KEY, notes = RETURNS_JSON_STRING_OF_DATSET_AS_PER_THE_DATASET_KEY_PROVIDED, response = String.class)
	...
	public DatasetModel getDataSet(@PathVariable(DATASET_KEY) String dataSetKey) {
		logger.debug("getDataSet ", dataSetKey);
		return new DatasetModel();
	}

	
	@ApiOperation(value = RESPOND_WITH_SAVED_DATASET_DETAILS, notes = RETURNS_A_JSON_STRING_WHICH_PROVIDES_INFORMATION_AFTER_SAVING_THE_DATASET, response = String.class)
	@ApiResponses(value = { @ApiResponse(code = 201, message = A_SUCCESSFUL_RESPONSE_AND_A_NEW_RESOURCE_WAS_CREATED),
			@ApiResponse(code = 400, message = BAD_REQUEST), @ApiResponse(code = 401, message = UNAUTHORIZED),
			@ApiResponse(code = 403, message = FORBIDDEN), @ApiResponse(code = 404, message = NOT_FOUND),
			@ApiResponse(code = 500, message = INTERNAL_SERVER_ERROR) })
	@RequestMapping(method = RequestMethod.POST)
	public DatasetModel saveDataSetDetail(@RequestBody DatasetModelPost dataSetObject) {
		logger.debug("saveDataSetDetail ", dataSetObject.toString());
		return new DatasetModel();
	}

	
	@ApiOperation(value = RESPOND_WITH_UPDATION_OF_DATASET_DETAILS, notes = RETURNS_A_HTTP_STATUS_CODE_TO_UPDATE_THE_EXISTING_RESOURCE_WITHOUT_CREATING_A_NEW_RESOURCE, response = String.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = A_SUCCESSFUL_RESPONSE_WITH_A_MESSAGE_BODY2),
			@ApiResponse(code = 400, message = BAD_REQUEST), @ApiResponse(code = 401, message = UNAUTHORIZED),
			@ApiResponse(code = 403, message = FORBIDDEN), @ApiResponse(code = 404, message = NOT_FOUND),
			@ApiResponse(code = 500, message = INTERNAL_SERVER_ERROR) })
	@RequestMapping(value = "/{datasetKey}", method = RequestMethod.PUT)
	public DatasetModel updateDataSetDetail(@PathVariable(DATASET_KEY) String dataSetKey, @RequestBody DatasetModelPut dataSet) {
		logger.debug("updateDataSetDetail ", dataSetKey);
		return new DatasetModel();
	}

	
	@ApiOperation(value = RESPOND_WITH_DELETION_DATASET_DETAILS, notes = RETURNS_A_HTTP_STATUS_CODE_ACCEPTING_DELETION_REQUEST_TO_DELETE_THE_EXISTING_RESOURCE, response = String.class)
	...
	public DatasetModel deleteDataSetDetail(@PathVariable(DATASET_KEY) String dataSetKey) {
		logger.debug("deleteDataSetDetail ", dataSetKey);
		return new DatasetModel();
	}

	
	@ApiOperation(value = RESPOND_LIST_OF_DATASOURCES_ASSOCIATED_WITH_A_DATASET, notes = RETURNS_A_JSON_ARRAY_THAT_DENOTES_DATASOURCE_ASSOCIATED_WITH_A_DATASET, response = String.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = A_SUCCESSFUL_RESPONSE_WITH_A_MESSAGE_BODY2),
			@ApiResponse(code = 400, message = BAD_REQUEST), @ApiResponse(code = 401, message = UNAUTHORIZED),
	...
	public List<DatasetModel> getDataSources(@PathVariable(DATASET_KEY) String dataSetKey) {
		logger.debug("getDataSources ", dataSetKey);
		return new ArrayList<DatasetModel>();
	}

	
	@ApiOperation(value = RESPOND_DATASOURCE_DETAILS_ASSOCIATED_WITH_A_DATASET, notes = RETURNS_A_JSON_STRING_THAT_DENOTES_DATASOURCE_DETAILS_ASSOCIATED_WITH_A_DATASET, response = String.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = A_SUCCESSFUL_RESPONSE_WITH_A_MESSAGE_BODY2),
			@ApiResponse(code = 400, message = BAD_REQUEST), @ApiResponse(code = 401, message = UNAUTHORIZED),
			@ApiResponse(code = 403, message = FORBIDDEN), @ApiResponse(code = 404, message = NOT_FOUND),
			@ApiResponse(code = 500, message = INTERNAL_SERVER_ERROR) })
	@RequestMapping(value = "/{datasetKey}/datasources", method = RequestMethod.PUT)
	public DatasetModel updateDataSourceKey(@PathVariable(DATASET_KEY) String dataSetKey, @RequestBody DatasetDatasourceInfo dataSource) {
		logger.debug("updateDataSourceKey ", dataSetKey);
		return new DatasetModel();
	}


	@ApiOperation(value = RETURNS_ATTRIBUTE_META_DATA, notes = RETURNS_A_JSON_ARRAY_OF_ATTRIBUTE_META_DATA_FOR_THE_GIVEN_DATASET, response = String.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = A_SUCCESSFUL_RESPONSE_WITH_A_MESSAGE_BODY2),
			@ApiResponse(code = 400, message = BAD_REQUEST), @ApiResponse(code = 401, message = UNAUTHORIZED),
			@ApiResponse(code = 403, message = FORBIDDEN), @ApiResponse(code = 404, message = NOT_FOUND),
			@ApiResponse(code = 500, message = INTERNAL_SERVER_ERROR) })
	@RequestMapping(value = "/{datasetKey}/attributeMetaData", method = RequestMethod.GET)
	public List<DataMetaDataInfo> getAttributeMetaData(@PathVariable(DATASET_KEY) String dataSetKey) {
		logger.debug("getAttributeMetaData ", dataSetKey);
		return new ArrayList<DataMetaDataInfo>();
	}

	
	@ApiOperation(value = RESPOND_CONTENTS_OF_DATASOURCE_ASSOCIATED_WITH_DATASET, notes = RETURNS_A_CHARACTER_STREAM_OF_DATA_FROM_THE_DATSOURCE_ASSOCIATED_WITH_A_DATASET, response = String.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = A_SUCCESSFUL_RESPONSE_WITH_A_MESSAGE_BODY2),
			@ApiResponse(code = 400, message = BAD_REQUEST), @ApiResponse(code = 401, message = UNAUTHORIZED),
			@ApiResponse(code = 403, message = FORBIDDEN), @ApiResponse(code = 404, message = NOT_FOUND),
			@ApiResponse(code = 500, message = INTERNAL_SERVER_ERROR) })
	@RequestMapping(value = "/{datasetKey}/attributeMetaData", method = RequestMethod.PUT)
	public DataMetaDataInfo updateAttributeMetaData(@PathVariable(DATASET_KEY) String dataSetKey, @RequestBody DatasetAttributeMetaData attributeMetaData) {
		logger.debug("updateAttributeMetaData ", dataSetKey);
		return new DataMetaDataInfo();
	}
