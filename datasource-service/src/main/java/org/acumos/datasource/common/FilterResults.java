...
public class FilterResults {
	...
	public static List<String> returnDatasets(List<String> dbResults, int pgOffset, int pgLimit) throws Exception {
	...
	public static Response setPaginationRecord(int page, int perPage, int totalRecords, String baseURL, ResponseBuilder httpResponse) {
		...
		if(page != 1) {
			...
			sb.append(baseURL).append(GlobalKeys.PAGE_PARAM).append(1).append(GlobalKeys.PER_PAGE_PARAM).append(perPage);
			linkRecords.put("first", sb.toString());
		}
		
		if(page <= total) {
			sb = new StringBuilder();
			sb.append(baseURL).append(GlobalKeys.PAGE_PARAM).append(page).append(GlobalKeys.PER_PAGE_PARAM).append(perPage);
			...
			if(nextPage < total) {
				sb = new StringBuilder();
				sb.append(baseURL).append(GlobalKeys.PAGE_PARAM).append(nextPage).append(GlobalKeys.PER_PAGE_PARAM).append(perPage);
			...
			if(previousPage != 0) {
				sb = new StringBuilder();
				sb.append(baseURL).append(GlobalKeys.PAGE_PARAM).append(previousPage).append(GlobalKeys.PER_PAGE_PARAM).append(perPage);
		...
		if(page != total) {
			sb = new StringBuilder();
			sb.append(baseURL).append(GlobalKeys.PAGE_PARAM).append(total).append(GlobalKeys.PER_PAGE_PARAM).append(perPage);
