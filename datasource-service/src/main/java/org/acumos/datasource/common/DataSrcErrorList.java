...
public class DataSrcErrorList {

	private static Logger log = LoggerFactory.getLogger(DataSrcErrorList.class);
	...
	private DataSrcErrorList() {
		
	}
	
	
	public static DataSrcRestError buildError(ErrorListEnum error, String[] errorVariables, String userMessage, CmlpApplicationEnum app) {
		...
		DataSrcRestError restError = new DataSrcRestError();
	...
	public static DataSrcRestError buildError(Exception sysException, ErrorListEnum error, CmlpApplicationEnum app) {
	...
	private static String getRefId(Exception e) {
		...
		try {
            ...
            while((temp = bufReader.readLine()) != null){
                if(temp.indexOf("org.acumos.datasource") >= 0)
	...
	public static DataSrcRestError buildError(String jsonStr, String backendName) {
	...
	public static DataSrcRestError getCmlpRestError(HttpResponse response, String backendNameURL) {
		try {
			...
			return DataSrcErrorList.buildError(sb.toString(), backendNameURL);
			
		} catch (Exception e) {
			log.info("Issue in handling Http response from backend call. " + e.getMessage());
			return DataSrcErrorList.buildError(e, null, CmlpApplicationEnum.DATASOURCE);
		}
	}
}
