...
//		rowCount = getHiveRowCount(results);
	...
	private long calculateHiveResultSizeBySampling(ResultSet results, int rowCount) throws SQLException, CmlpDataSrcException {
	...
	private void restoreKerberosUserConfigFiles(String krbConfigFileName, String krbKeyTabFileName, Map<String, String> decryptionMap) throws CmlpDataSrcException {
		log.info("getSampleResults(), restoring config files from codecloud...");
		boolean isWritten = Utilities.writeToKerberosFile(krbConfigFileName, decryptionMap.get("KerberosConfigFileContents".toLowerCase()));
		
		if(isWritten) {
			isWritten = Utilities.writeToKerberosFile(krbKeyTabFileName, decryptionMap.get("KerberosKeyTabContent".toLowerCase()));
		}
		
		if(!isWritten) {
			//1. delete config files, if any
			Utilities.deleteUserKerberosConfigFiles(krbConfigFileName, krbKeyTabFileName);
	...
	@Override
	public InputStream getSampleResults(DataSourceModelGet dataSource) throws CmlpDataSrcException, IOException, SQLException, ClassNotFoundException {
	...
	if(!Files.exists(Paths.get(sb1.toString()))) {
		...
		//Remove semicolon
		dataSource.getHdfsHiveDetails().setQuery(Utilities.trimSemicolonAtEnd(dataSource.getHdfsHiveDetails().getQuery()));
