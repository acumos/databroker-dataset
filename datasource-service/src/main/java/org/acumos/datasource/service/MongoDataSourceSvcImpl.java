...
public class MongoDataSourceSvcImpl implements MongoDataSourceSvc {
	...
	public String getConnectionStatus_2_10(MongoDbConnectionModel objMongoDbConnectionModel, String query)
			throws UnknownHostException, IOException, CmlpDataSrcException {
		...
		if (objMongoDbConnectionModel.getHostname() != null && objMongoDbConnectionModel.getPort() != 0
				...
				//&& objMongoDbConnectionModel.getPassword() != null
				//&& objMongoDbConnectionModel.getUsername() != null
				) {
			...
			if (query != null) {
				...
				queryMap = objmapper.readValue(query, new TypeReference<HashMap<String, String>>() {
				});
				BasicDBObject query1 = new BasicDBObject();
				query1.putAll(queryMap);*/
	...
	public MongoClient getConnection(MongoDbConnectionModel objMongoDbConnectionModel)
			throws UnknownHostException, CmlpDataSrcException, IOException {
		MongoClient mongo = null;
		DB db = null;
		...
		if(mongo == null) {
			Utilities.raiseConnectionFailedException("mongo");
		}
	...
	public InputStream getResults(String user, String authorization, String namespace, String datasourceKey)
		...
		//JSONObject objJson = new JSONObject(mongoDetails.get(0));
		...
		if (dbDataSource.getCategory().equals("mongo") && dbDataSource.getOwnedBy().equals(user)) {
			...
			objMongoDbConnectionModel.setPassword(decrptionMap.get("dbServerPassword".toLowerCase()));
			objMongoDbConnectionModel.setPort(dbDataSource.getCommonDetails().getPortNumber());
			objMongoDbConnectionModel.setUsername(decrptionMap.get("dbServerUsername".toLowerCase()));
			...
			if(mongo != null) {
				...
				if(db != null) {
					/*// preparing query map based on query json provided
					Map<String, String> queryMap = new HashMap<>();
					ObjectMapper objmapper = new ObjectMapper();
					queryMap = objmapper.readValue(query, new TypeReference<HashMap<String, String>>() {
					});
		
					// initializing basicdbobject with query map
					BasicDBObject query1 = new BasicDBObject();
					query1.putAll(queryMap);*/
	...
	private void saveMetadata(MongoDbConnectionModel objMongoDbConnectionModel,
							 DB db,double size, int numberOfDocs,BasicDBObject query1) throws CmlpDataSrcException
    {
...
//        m.setListIndex(indexList);
	...
	public InputStream getSampleResults (String user, String authorization, String namespace, String datasourceKey)
		...
		List<String> dbDatasourceDetails = dbUtilities.
				getDataSourceDetails(user, null, null,
						datasourceKey, null, true, false, authorization);
		/*JSONObject objJson = null;
		if(mongoDetails.size() > 0) {
			objJson = new JSONObject(mongoDetails.get(0));
		}*/
	...
	@Override
	public InputStream getSampleResults(DataSourceModelGet dataSource) throws UnknownHostException, IOException, NumberFormatException, CmlpDataSrcException {
		...
		// initializing a query
		log.info("getSampleResults, getting query from the objconnectionmodel payload provided");
//		String query = objJson.getString("query");
		...
		if(mongo != null) {
			...
			if(db != null) {
				...
				Map<String, String> queryMap = new HashMap<>();
				ObjectMapper objmapper = new ObjectMapper();
				queryMap = objmapper.readValue(dataSource.getDbDetails().getDbQuery(), new TypeReference<HashMap<String, String>>() {
				});
