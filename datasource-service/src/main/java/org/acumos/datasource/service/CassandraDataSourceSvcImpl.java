...
public class CassandraDataSourceSvcImpl implements CassandraDataSourceSvc {
	...
	public String getConnectionStatus(CassandraConnectionModel objCassandraConnectionModel, String query)
			throws CmlpDataSrcException, IOException, SQLException {
		...
		query = "SELECT * FROM cmlp.entitytypes;";*/
		...
		if (objCassandraConnectionModel.getNode() != null && objCassandraConnectionModel.getPassword() != null
			...
			if (session != null && query == null) {
				...
				// HARD CODING QUERY FOR TESTING
				// BLOCKING ON
				// https://itrack.web.att.com/browse/CPFCE-2742
	...
	public String getConnectionStatus(String node, int port, String keyspaceName) throws CmlpDataSrcException {
		...
		if (node != null && port != 0) {
			...
			//throw new CmlpDataSrcException("Please check input parameters");
			raiseInvalidInputParameters();
		}
	...
	public void createkeySpace(Session session, String keySpaceName, String replicationStrategy,
			int replicationFactor) throws CmlpDataSrcException {
	...
	public InputStream getResults(String user, String authorization, String namespace, String datasourceKey)
		...
		if (dbDataSource.getCategory().equals("cassandra") && dbDataSource.getOwnedBy().equals(user)) {
			...
			session = getSession(dbDataSource.getCommonDetails().getServerName(), dbDataSource.getCommonDetails().getPortNumber(),
					decrptionMap.get("dbServerUsername".toLowerCase()),
			...
			for (Row row : results) {
				for (int i = 0; i < columnCount; i++) {
					...
					if (columnType.equals("ASCII") || columnType.equals("BLOB") || columnType.equals("INET")
						...
						//		"OOPS. The datatype has not been configured. Please raise a bug for mapping.");
	...
	private DataSourceMetadata getCassandraMetadata(ResultSet results, int rowCount) throws CmlpDataSrcException, IOException, SQLException {
	...
	public InputStream getSampleResults(String user, String authorization, String namespace, String datasourceKey)
		...
		//log.info("CassandraDataSourceSvcImpl::getSampleResults\n: " + inputStreamToString(result));
	...
	private InputStream getDatasourceSampleData(String user, String authorization, String datasourceKey, int sampleSize)
		...
		//log.info( "CassandraDataSourceSvcImpl::getDatasetSampleData: objJson:\n" + objJson);
		if (dbDataSource.getCategory().equals("cassandra")
			...
			//throw new CmlpDataSrcException("please check user permission and datasourcekey");
			String[] variables = {"datasourceKey"};
			CmlpRestError err = CmlpErrorList.buildError(ErrorListEnum._0003, variables, null, CmlpApplicationEnum.DATASOURCE);
	...
	private String inputStreamToString(InputStream inputStream) throws IOException {
		log.info( "ENTER:CassandraDataSourceSvcImpl::inputStreamToString");
		 ...
		 StringBuilder textBuilder = new StringBuilder();
			...
			log.info( "RETURN:CassandraDataSourceSvcImpl::inputStreamToString");
		    return textBuilder.toString();
	}

	private StringBuilder buildSampleData(ResultSet results, int sampleSize) throws CmlpDataSrcException {
	...
	public long getCassandraDatasetSize(ResultSet results, int rowCount) throws SQLException, IOException, CmlpDataSrcException {
	...
	private long calculateCassandraDatasetSizeBySampling(ResultSet results, int rowCount) throws SQLException, CmlpDataSrcException {		
	...
	private long calculateCassandraDatasetSizeByNotSampling(ResultSet results, int rowCount)
			throws SQLException, CmlpDataSrcException {
	...
	@Override
	public InputStream getSampleResults(DataSourceModelGet dataSource) throws CmlpDataSrcException {
	...
	private void raiseInvalidInputParameters() throws CmlpDataSrcException {
