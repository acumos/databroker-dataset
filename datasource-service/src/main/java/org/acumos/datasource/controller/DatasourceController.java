...
public class DatasourceController {
	...
	public Response getDataSourceSamples(@RequestHeader(GlobalKeys.AUTHORIZATION2) String authorization,
		...
		try {
			...
			InputStream in = service.getDataSourceSamples(remoteUser, authorization, dataSourceKey, null);
			
			if(in != null) {
				Response response = Response.status(200).entity(in).build();
				response.getHeaders().add("Content-Type", ApplicationUtilities.getResponseContentType(dataSourceKey));
				return response;
			} else {
				...
				throw new DataSrcException("No Datasource Samples available for the given datasourceKey.",
						Status.NOT_FOUND.getStatusCode(), err);
			}
